...
/**
 * This processor enables incremental updates (aka near-realtime updates) for
 * AnalyzingInfixSuggester. It is tied to a spell check component and discovers
 * any MultiSuggesters configured for that component.
 */
public class MultiSuggesterProcessor extends UpdateRequestProcessor {
    ...
    public void processCommit(CommitUpdateCommand cmd) throws IOException {
        for (MultiSuggester suggester : suggesters) {
            suggester.commit(cmd.getReq().getSearcher());
        }
        if (next != null) {
