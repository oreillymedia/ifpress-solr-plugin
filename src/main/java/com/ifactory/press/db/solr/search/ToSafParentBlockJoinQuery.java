...
/**
 * This query requires that you index children and parent docs as a single
 * block, using the {@link IndexWriter#addDocuments IndexWriter.addDocuments()}
 * or {@link
 ...
 * document. At search time you provide a Filter identifying the parents,
 * however this Filter must provide an {@link BitSet} per sub-reader.
 *
 * <p>
 * Once the block index is built, use this query to wrap any sub-query matching
 * only child docs and join matches in that child document space up to the
 * parent document space. You can then use this Query as a clause with other
 * queries in the parent document space.</p>
 *
 * <p>
 ...
 * child query must never return a parent document.</p>
 *
 * <p>
 * See {@link org.apache.lucene.search.join} for an overview. </p>
 *
 * @lucene.experimental
 */
public class ToSafParentBlockJoinQuery extends Query {
   ...
   *
     */
    public ToSafParentBlockJoinQuery(Query childQuery, BitSetProducer parentsFilter, ScoreMode scoreMode) {
        super();
    ...
    private boolean equalsTo(ToSafParentBlockJoinQuery other) {
