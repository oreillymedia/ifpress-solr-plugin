...
import org.apache.solr.spelling.Token; //changed from Lucene
 ...
 * zero-weighted based on a global minimum <b>threshold</b> term frequency, a
 * per-field minimum threshold (<b>minfreq</b>) and a per-field maximum
 * (<b>maxfreq</b>) threshold. All thresholds are compared against (term
 * frequency / document count), an estimate of the fraction of documents
 * containing the term. Thus setting maximum=0.5 will filter out terms occurring
 * in as many or more than half the documents.
 * </p>
 *
 * <p>
 * In the normal mode of operation, the given <b>field</b>'s analyzer is used to
 * tokenize the stored field values; each resulting token becomes a suggestion.
 * </p>
 *
 * <p>
 ...
 * <b>analyzerFieldType</b>=string in the suggester configuration. In this mode,
 * every suggestion is given the field's constant <b>weight</b>: term frequency
 * is not considered as part of the weight, and no filtering is applied based on
 * frequency.
 * </p>
 *
 * <p>
 * If <b>filterDuplicates</b> is set to true for a field, then each suggestion
 * generated by the field is looked up in the suggester: if it exists already,
 * it is not added again. This slows down indexing considerably so it should
 * only be turned on when actually needed.
 * </p>
 *
 * <p>
...
public class MultiSuggester extends Suggester {
  ...
  // use a synchronized Multimap - there may be one with the same name for each
     ...
     *
     * suggestions for each field are managed using one of the following
     * weighting and update strategies: - constant weight: all terms occurring
     * in the field are weighted equally - frequency weight: terms have a weight
     * that is the field's weight * the number of occurrences frequency-weighted
     * suggestions can have their frequency calculated by: - the value of
     * docFreq() on a source field - a frequency maintained in a docValues field
